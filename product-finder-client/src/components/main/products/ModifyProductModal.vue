          ...
          <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
            <div class="sm:flex sm:items-start ">
              <div class=" w-full text-center sm:text-left">
                <div class="m-2">
                    <h3 class="text-base font-semibold leading-6 text-gray-900"id="modal-title">Modificar Producto</h3>
                    <p class="text-xs text-gray-500">
                        Rellene los campos a modificar.
                    </p>
                </div>
...
async function registerHandler(){
    if(isFormValid.value){
        try{
            ...
            const response = await productUpdate(props.product.id, name.value, sku.value, description.value, price.value, photos.value);
            //isSubmitting.value=false;
            console.log(response);
    
            if(response.data['success'] === 'true'){
                message.value = "Producto modificado exitosamente.";
        ...
        message.value="Rellene al menos un campo a modificar";
        productAddSuccess.value=false;
        setTimeout(() => {
                    window.location.reload()
                }, 1200 );
